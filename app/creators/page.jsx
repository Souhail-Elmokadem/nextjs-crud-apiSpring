...
export default function page() {
    ...
    useEffect(() => {
        ...
        const handleSearch = async () => {
            try {
                    const response = await fetch(`http://localhost:8089/api/allCreators?page=${currentPage}&Search=${searchTerm}`);
        ...
        const handle = async() => {
            ...
            try {
                if(search !== null){
                    ...
                    const response = await fetch(`http://localhost:8089/api/allCreators?page=${search}`);
                    const res = await response.json();
    ...
    return (
        <>
            <Hero>
                <div className="relative overflow-x-auto shadow-md sm:rounded-lg" style={{ minWidth: '950px', transform: 'translateX(-200px)' }}>
                    <div className="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
                        <div>
                            <button id="dropdownRadioButton" data-dropdown-toggle="dropdownRadio" onClick={() => drpdown ? setDrpdown(false) : setDrpdown(true)} className="inline-flex items-center text-gray-500 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-3 py-1.5 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700" type="button">
                            ...
                            <div id="dropdownRadio" className={`z-10 w-48 ${drpdown ? '' : 'hidden'}  bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700 dark:divide-gray-600`} data-popper-reference-hidden="dropdownRadio" data-popper-escaped="dropdownRadio" data-popper-placement="top"
                                style={{
                                    'position': 'absolute',
                                }}>
                        ...
                        <div className="relative">
                            ...
                            <input type="text" onChange={(e) => setSearchTerm(e.target.value)} id="table-search" className="block p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for items" />
                        </div>
                    </div>

                    <creatorContext.Provider value={{ setCreators }} >
                        <Table data={creators} />
